...
if exist "%TBBROOT%\..\..\linux\tbb\lib\intel64\gcc4.8\libtbb.so.2" (
    set "LIBRARY_PATH=%TBBROOT%\..\..\linux\tbb\lib\intel64\gcc4.8;%LIBRARY_PATH%"
    set "LD_LIBRARY_PATH=%TBBROOT%\..\..\linux\tbb\lib\intel64\gcc4.8;%LD_LIBRARY_PATH%"
)

:: Use Intel compiler if it is in environment. Otherwise MSVS compiler will be used
where /q icl.exe
if %errorlevel% == 0 (
    set "TBB_CXX=icl.exe"
)
