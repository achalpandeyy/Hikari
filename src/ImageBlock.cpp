...
void ImageBlock::Put(const ImageBlock& block)
{
    ...
    unsigned int maxY = std::min((block.m_Position.y + 1) * BLOCK_DIMENSION, m_Dimensions.y);
    unsigned int maxX = std::min((block.m_Position.x + 1) * BLOCK_DIMENSION, m_Dimensions.x);
