...
glm::vec3 WhittedIntegrator::Li(const Ray& ray, const Scene& scene, unsigned int rayDepth) const
{
    ...
    for (size_t i = 0; i < scene.m_Lights.size(); i++)
    {
        ...
        glm::vec3 diffuse = (interaction.Shading.m_Albedo / glm::vec3(M_PI)) * (scene.m_Lights[i]->GetIncidentLight(interaction.m_HitPoint)) * std::max(0.f, glm::dot(lightDirection, interaction.m_Normal) * static_cast<int>(!inShadow));
