...
glm::vec3 WhittedIntegrator::Li(const Ray& ray, const Scene& scene, unsigned int rayDepth) const
{
    ...
    glm::vec3 hitPoint = ray.m_Origin + intersection.m_Distance * ray.m_Direction;
    ...
    for (size_t i = 0; i < scene.m_Lights.size(); i++)
    {
        ...
        switch (scene.m_Lights[i]->m_LightType)
        {
            case Light::E_POINT_LIGHT:
                lightDirection = glm::normalize(scene.m_Lights[i]->m_Position - hitPoint);
                distanceToLight = glm::distance(hitPoint, scene.m_Lights[i]->m_Position);
                break;
        ...
        Ray shadowRay(hitPoint + intersection.m_Normal * bias, lightDirection, distanceToLight);
        ...
        glm::vec3 diffuse = (intersection.m_Albedo / glm::vec3(M_PI)) * (scene.m_Lights[i]->GetIncidentLight(hitPoint)) * std::max(0.f, glm::dot(lightDirection, intersection.m_Normal) * static_cast<int>(!inShadow));
