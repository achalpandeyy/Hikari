...
namespace Hikari
{
    ...
    {
        std::vector< std::pair<float, float> > sortVector(count);
        for (unsigned int i = 0u; i < count; ++i)
        {
            sortVector[i] = std::make_pair(wavelengths[i], values[i]);
        }
        ...
        for (unsigned int i = 0u; i < count; ++i)
        {
            wavelengths[i] = sortVector[i].first;
            values[i] = sortVector[i].second;
        }
    }
    ...
    float AverageSpectrumSamples(
        ...
        float           wavHigh)
    {
        // Log check for sorted wavelenghts
        // Log check for wavLow < wavHigh

        // Given range lies completely outside the `wavelengths`
        if (wavHigh <= wavelengths[0])
            return values[0];
        if (wavLow >= wavelengths[count - 1])
            return values[count - 1];

        float sum = 0.f;
        // Given range partly lie inside the `wavelengths`
        if (wavLow < wavelengths[0])
            sum += values[0] * (wavelengths[0] - wavLow);
        if (wavHigh > wavelengths[count - 1])
            sum += values[count - 1] * (wavHigh - wavelengths[count - 1]);

        // For the range which lie completely inside `wavelengths`
        unsigned int i = 0u;
        while (wavLow > wavelengths[i + 1]) ++i;
        // Log check i + 1 < n

        auto interp = [wavelengths, values](float w, unsigned int i)
        {
            return glm::lerp(values[i], values[i + 1],
                (w - wavelengths[i]) / (wavelengths[i + 1] - wavelengths[i]));
        };

        for (; i + 1 < count && wavHigh >= wavelengths[i]; ++i)
        {
            float segmentWavStart = std::max(wavLow, wavelengths[i]);
            float segmentWavEnd = std::min(wavHigh, wavelengths[i + 1]);
            sum += 0.5f * (interp(segmentWavStart, i) + interp(segmentWavEnd, i))
                * (segmentWavEnd - segmentWavStart);
        }
        return sum / (wavHigh - wavLow);
    }
    ...
    SampledSpectrum SampledSpectrum::FromSampledSPD(
        const float* wavelengths,
        const float* values,
        unsigned int    count)
    {
        // Sort samples if unordered
        if (!SpectrumSamplesSorted(wavelengths, values, count))
        {
            std::vector<float> sortedWavelengths(&wavelengths[0], &wavelengths[count]);
            std::vector<float> sortedValues(&values[0], &values[count]);
            SortSpectrumSamples(sortedWavelengths.data(), sortedValues.data(), count);
            return FromSampledSPD(&sortedWavelengths[0], &sortedValues[0], count);
        }

        SampledSpectrum spectrum;
        for (unsigned int i = 0u; i < s_NumSamples; ++i)
        {
            // Compute average value of given SPD over ith sample's range
            float wavLow = glm::lerp(SAMPLED_WAVELENGTH_START,
                SAMPLED_WAVELENGTH_END, float(i) / float(s_NumSamples));
            float wavHigh = glm::lerp(SAMPLED_WAVELENGTH_START,
                SAMPLED_WAVELENGTH_END, float(i + 1) / float(s_NumSamples));
            spectrum.m_Coeffs[i] = AverageSpectrumSamples(wavelengths, values, count,
                wavLow, wavHigh);
        }
        return spectrum;
    }
    ...
    SampledSpectrum SampledSpectrum::FromRGB(const float rgb[3], SpectrumType type)
    {
        ...
        if (type == SpectrumType::Reflectance)
        {
            ...
            {
                spectrum += rgb[0] * s_RgbRefl2SpectWhite;

                if (rgb[1] <= rgb[2])
                {
                    spectrum += (rgb[1] - rgb[0]) * s_RgbRefl2SpectCyan;
                    spectrum += (rgb[2] - rgb[1]) * s_RgbRefl2SpectBlue;
                }
                else
                {
                    spectrum += (rgb[2] - rgb[0]) * s_RgbRefl2SpectCyan;
                    spectrum += (rgb[1] - rgb[2]) * s_RgbRefl2SpectGreen;
                }
            }
            else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2])
            {
                spectrum += rgb[1] * s_RgbRefl2SpectWhite;

                if (rgb[0] <= rgb[2])
                {
                    spectrum += (rgb[0] - rgb[1]) * s_RgbRefl2SpectMagenta;
                    spectrum += (rgb[2] - rgb[0]) * s_RgbRefl2SpectBlue;
                }
                else
                {
                    spectrum += (rgb[2] - rgb[1]) * s_RgbRefl2SpectMagenta;
                    spectrum += (rgb[0] - rgb[2]) * s_RgbRefl2SpectRed;
                }
            }
            else
            {
                spectrum += rgb[2] * s_RgbRefl2SpectWhite;

                if (rgb[0] <= rgb[1])
                {
                    spectrum += (rgb[0] - rgb[2]) * s_RgbRefl2SpectYellow;
                    spectrum += (rgb[1] - rgb[0]) * s_RgbRefl2SpectGreen;
                }
                else {
                    spectrum += (rgb[1] - rgb[2]) * s_RgbRefl2SpectYellow;
                    spectrum += (rgb[0] - rgb[1]) * s_RgbRefl2SpectRed;
                }
            }
            spectrum *= .94f;
        }
        else
        {
            if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2])
            {
                spectrum += rgb[0] * s_RgbIllum2SpectWhite;

                if (rgb[1] <= rgb[2])
                {
                    spectrum += (rgb[1] - rgb[0]) * s_RgbIllum2SpectCyan;
                    spectrum += (rgb[2] - rgb[1]) * s_RgbIllum2SpectBlue;
                }
                else
                {
                    spectrum += (rgb[2] - rgb[0]) * s_RgbIllum2SpectCyan;
                    spectrum += (rgb[1] - rgb[2]) * s_RgbIllum2SpectGreen;
                }
            }
            else if (rgb[1] <= rgb[0] && rgb[1] <= rgb[2])
            {
                spectrum += rgb[1] * s_RgbIllum2SpectWhite;
                if (rgb[0] <= rgb[2])
                {
                    spectrum += (rgb[0] - rgb[1]) * s_RgbIllum2SpectMagenta;
                    spectrum += (rgb[2] - rgb[0]) * s_RgbIllum2SpectBlue;
                }
                else
                {
                    spectrum += (rgb[2] - rgb[1]) * s_RgbIllum2SpectMagenta;
                    spectrum += (rgb[0] - rgb[2]) * s_RgbIllum2SpectRed;
                }
            }
            else
            {
                spectrum += rgb[2] * s_RgbIllum2SpectWhite;
                if (rgb[0] <= rgb[1])
                {
                    spectrum += (rgb[0] - rgb[2]) * s_RgbIllum2SpectYellow;
                    spectrum += (rgb[1] - rgb[0]) * s_RgbIllum2SpectGreen;
                }
                else
                {
                    spectrum += (rgb[1] - rgb[2]) * s_RgbIllum2SpectYellow;
                    spectrum += (rgb[0] - rgb[1]) * s_RgbIllum2SpectRed;
                }
            }
            spectrum *= .86445f;
        }
        return spectrum.Clamp();
    }
    ...
    {
        // Compute X, Y, Z matching function from the hardcoded CIE dataset
        for (unsigned int i = 0u; i < s_NumSamples; ++i)
        {
            float wavLow = glm::lerp(SAMPLED_WAVELENGTH_START, SAMPLED_WAVELENGTH_END,
                float(i) / float(s_NumSamples));
            float wavHigh = glm::lerp(SAMPLED_WAVELENGTH_START, SAMPLED_WAVELENGTH_END,
                float(i + 1) / float(s_NumSamples));

            s_X.m_Coeffs[i] = AverageSpectrumSamples(CIE_wavelengths, CIE_X,
                CIE_NumSamples, wavLow, wavHigh);
            s_Y.m_Coeffs[i] = AverageSpectrumSamples(CIE_wavelengths, CIE_Y,
                CIE_NumSamples, wavLow, wavHigh);
            s_Z.m_Coeffs[i] = AverageSpectrumSamples(CIE_wavelengths, CIE_Z,
                CIE_NumSamples, wavLow, wavHigh);
        }

        // Compute RGB to spectrum functions from Smits (1999) dataset
        for (unsigned int i = 0u; i < s_NumSamples; ++i)
        {
            float wl0 = glm::lerp(SAMPLED_WAVELENGTH_START, SAMPLED_WAVELENGTH_END,
                float(i) / float(s_NumSamples));
            float wl1 = glm::lerp(SAMPLED_WAVELENGTH_START, SAMPLED_WAVELENGTH_END,
                float(i + 1) / float(s_NumSamples));

            s_RgbRefl2SpectWhite.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectWhite, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectCyan.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectCyan, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectMagenta.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectMagenta, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectYellow.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectYellow, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectRed.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectRed, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectGreen.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectGreen, numRGB2SpectSamples, wl0, wl1);

            s_RgbRefl2SpectBlue.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBRefl2SpectBlue, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectWhite.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectWhite, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectCyan.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectCyan, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectMagenta.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectMagenta, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectYellow.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectYellow, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectRed.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectRed, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectGreen.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectGreen, numRGB2SpectSamples, wl0, wl1);

            s_RgbIllum2SpectBlue.m_Coeffs[i] = AverageSpectrumSamples(RGB2SpectLambda,
                RGBIllum2SpectBlue, numRGB2SpectSamples, wl0, wl1);
        }
    }
    ...
    void SampledSpectrum::ToXYZ(float xyz[3]) const
    {
        ...
        for (unsigned int i = 0u; i < s_NumSamples; ++i)
        {
            xyz[0] += s_X[i] * m_Coeffs[i];
            xyz[1] += s_Y[i] * m_Coeffs[i];
            xyz[2] += s_Z[i] * m_Coeffs[i];

        }

        float scale = (SAMPLED_WAVELENGTH_END - SAMPLED_WAVELENGTH_START) /
            float(s_NumSamples * CIE_YIntegral);
    ...
    float SampledSpectrum::y() const
    {
        ...
        for (unsigned int i = 0u; i < s_NumSamples; ++i)
        {
            yy += s_Y.m_Coeffs[i] * m_Coeffs[i];
        }

        return yy * ((SAMPLED_WAVELENGTH_END - SAMPLED_WAVELENGTH_START) / float(s_NumSamples));
    }
    ...
    float InterpolateSpectrumSamples(const float* wavelengths, const float* values, unsigned int count, const float wl)
    {
        if (wl <= wavelengths[0])
            return values[0];
        ...
        int offset = FindInterval(count, [&](int index) {return wavelengths[index] <= wl; });

        float t = (wl - wavelengths[offset]) / (wavelengths[offset + 1] - wavelengths[offset]);
        return glm::lerp(values[offset], values[offset + 1], t);
    }

    RGBSpectrum RGBSpectrum::FromRGB(const float rgb[3], SpectrumType type)
    {
        RGBSpectrum spectrum;
        spectrum.m_Coeffs[0] = rgb[0];
        spectrum.m_Coeffs[1] = rgb[1];
        spectrum.m_Coeffs[2] = rgb[2];
        // Log check if spectrum has NaNs
        return spectrum;
    }

    RGBSpectrum RGBSpectrum::FromSampledSPD(const float* wavelengths, const float* values, unsigned int count)
    {
        // Sort samples if unordered
        if (!SpectrumSamplesSorted(wavelengths, values, count))
        {
        ...
        for (unsigned int i = 0u; i < CIE_NumSamples; ++i)
        {
            float val = InterpolateSpectrumSamples(wavelengths, values, count, CIE_wavelengths[i]);
        }
    }

    void RGBSpectrum::ToRGB(float rgb[3]) const
    {
        rgb[0] = m_Coeffs[0];
        rgb[1] = m_Coeffs[1];
        rgb[2] = m_Coeffs[2];
    ...
    {
        RGBSpectrum spectrum;
        XYZToRGB(xyz, spectrum.m_Coeffs);
        return spectrum;
    }
    ...
    float RGBSpectrum::y() const
    {
        ...
        return YWeight[0] * m_Coeffs[0] + YWeight[1] * m_Coeffs[1] + YWeight[2] * m_Coeffs[2];
    }
    ...
    SampledSpectrum SampledSpectrum::s_X;
    SampledSpectrum SampledSpectrum::s_Y;
    SampledSpectrum SampledSpectrum::s_Z;

    // CIE X function values
    const float CIE_X[CIE_NumSamples] =
    {
    ...
    const float CIE_Y[CIE_NumSamples] =
    {
    ...
    const float CIE_Z[CIE_NumSamples] =
    {
        0.0006061000f,
        0.0006808792f,
        0.0007651456f,
    ...
    const float CIE_wavelengths[CIE_NumSamples] =
    {
    ...
    // Smits (1999) Datasets
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectWhite;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectCyan;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectMagenta;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectYellow;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectRed;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectGreen;
    SampledSpectrum SampledSpectrum::s_RgbRefl2SpectBlue;

    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectWhite;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectCyan;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectMagenta;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectYellow;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectRed;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectGreen;
    SampledSpectrum SampledSpectrum::s_RgbIllum2SpectBlue;

    const float RGB2SpectLambda[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectWhite[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectCyan[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectMagenta[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectYellow[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectRed[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectGreen[numRGB2SpectSamples] =
    {
    ...
    const float RGBRefl2SpectBlue[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectWhite[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectCyan[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectMagenta[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectYellow[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectRed[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectGreen[numRGB2SpectSamples] =
    {
    ...
    const float RGBIllum2SpectBlue[numRGB2SpectSamples] =
    {
